    ...
    <details name="forms">
      ...
      <form className="form-wrapper" onSubmit={saveForm}>
        {status === "add" && (
          <>
            {inputs.fields.map((input, index) => (
              <div>
                <label key={input.key} htmlFor={input.name}>
                  {input.label}
                </label>
                <input
                  autoFocus={index === 0}
            ...
            <div className="form-btn">
              <button type="button" onClick={() => setStatus("close")}>
                Cancel
              </button>
            ...
            {formData.map((form) => (
              <li tabIndex={0}>
                <button
                  ...
                  onClick={showForm}
                  data-id={form.id}
                  aria-description="edit entry"
                >
                  {form[inputs.displayField]}
                </button>
              </li>
            ))}
          </ul>
        ...
        {status !== "edit" && status !== "add" && (
          <button
            id="add-btn"
            aria-labelledby={"add-btn " + summaryId}
